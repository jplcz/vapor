...
{
    template <typename E>
    struct VAPOR_NODISCARD Unexpected
    {
        E error;
    ...
    class VAPOR_NODISCARD Expected
    {
    private:
        union
        {
    ...
    public:
        ...
        VAPOR_CXX14_CONSTEXPR Expected(const Unexpected<E>& unex) : m_has_value(false)
        {
        ...
        VAPOR_CXX14_CONSTEXPR Expected(const Expected& other) : m_has_value(other.m_has_value)
        {
            if (m_has_value)
            {
        ...
        VAPOR_CXX14_CONSTEXPR Expected(Expected&& other) noexcept : m_has_value(other.m_has_value)
        {
            if (m_has_value)
            {
                new(&m_value) T((other.m_value));
        ...
        VAPOR_CXX14_CONSTEXPR Expected(const std::expected<T, E>& other) : m_has_value(other.has_value())
        {
            if (m_has_value)
            {
        ...
        VAPOR_CXX14_CONSTEXPR Expected(std::expected<T, E>&& other) : m_has_value(other.has_value())
        {
            if (m_has_value)
            {
                new(&m_value) T((other.value()));
        ...
        VAPOR_NODISCARD VAPOR_CXX14_CONSTEXPR bool has_value() const noexcept { return m_has_value; }
        VAPOR_NODISCARD VAPOR_CXX14_CONSTEXPR explicit operator bool() const noexcept { return m_has_value; }
        ...
        VAPOR_NODISCARD VAPOR_CXX14_CONSTEXPR const T* operator->() const noexcept { return &m_value; }
        VAPOR_NODISCARD VAPOR_CXX14_CONSTEXPR T* operator->() noexcept { return &m_value; }
